...
@WebServlet("/article")
public class BoardController_Servelt extends HttpServlet {
	...
	private String board(HttpServletRequest request, HttpServletResponse response) {
	...
	private String modify(HttpServletRequest request, HttpServletResponse response) {
		...
		if (memberDto != null) {
			...
			try {
				...
				return "/article?action=board&pgno=1&key=&word=";
			} catch (Exception e) {
				e.printStackTrace();
	...
	private String delete(HttpServletRequest request, HttpServletResponse response) {
		...
		// TODO : 글삭제 완료 후 board.jsp로 이동.(이후의 프로세스를 생각해 보세요.)
		HttpSession session = request.getSession();
		MemberDto memberDto = (MemberDto) session.getAttribute("userinfo");
		if (memberDto != null) {
			int articleNo = Integer.parseInt(request.getParameter("articleno"));
